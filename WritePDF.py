...
# by Ricardo IlGrande and Hector Simosa
# To add images befor header.
...
def main():
	...
	# PDF header string
	pag_opt["hdr_str"] = \
	    string.ljust("Relacion archivos Inscripcion." , 20) + \
		string.center("Fecha: " + pag_opt["gen_date_time"], 40)
...
def write_pdf(file_info, pag_opt):
	...
	pag_set["top_y"] = 10.5 * inch         # y-coord for 1st line on each page
	...
	# write the header for the first page, before the main loop. Insert an image before hdr.
	canv.drawInlineImage("sartmu.png", 130,760)
	...
	for lin in txt_fil:
		...
		canv.drawString(pag_set["x"], pag_set["y"], lin.rstrip())
...
def write_hdr(file_info, pag_opt, pag_set):
	...
	hdr_lin = pag_opt["hdr_str"] + \
		string.rjust("Page: " + str(pag_set["page_num"]), 10)
